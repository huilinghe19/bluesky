...
geek_list = ["Select a test signal...", testdevice1.name, keithley6517_current.name]
m1 = ophyd.EpicsMotor('IOCsim:m1', name="m1", labels=("motors",))

def getEpicsPV(ophyd_dev):
    dict_map = {"devices": {"m1": "IOCsim:m1", "m2": "IOCsim:m2"}}
    if ophyd_dev in dict_map["devices"].keys():
        return dict_map["devices"][ophyd_dev]
    else:
        return None
    
def getEpicsSignal(ophyd_sig):
    dict_map = {"signals": {"keithley6517_current": "EMILEL:test:rdCurE3"}}
    if ophyd_sig in dict_map["signals"].keys():
        return dict_map["signals"][ophyd_sig]
    else:
        return None
        
def plan():
    yield from scan([det], m1, -1, 1, 4)
    
def plan1():
    yield from scan([keithley6517_current], m1, -4, 4, 4)


class Widget(QWidget):
    def __init__(self, model, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.model = AutoLines(max_runs=2)
        self.RE = RunEngine()
        self.RE.subscribe(stream_documents_into_runs(self.model.add_run))
        self.view = QtFigures(self.model.figures) 
        ...
        #self.v_box.addWidget(self.lineEdit)
        ...
        #self.lableLayout2 = QVBoxLayout()
        self.parametersLayout= QVBoxLayout()
        #self.n_motors_layout = QVBoxLayout()
        
        self.h_box.addLayout(self.lableLayout)
        self.h_box.addLayout(self.parametersLayout)
        #self.h_box.addLayout(self.lableLayout2)
        #self.h_box.addLayout(self.n_motors_layout)
        ...
        self.lableLayout.addWidget(self.combo)
        ...
        self.parametersLayout.addWidget(self.button)
    ...
    def on_clicled(self):      
        if self.current_text == "det":
            ...
            print("keithley6517_current is chosen, scan keithley6517 current")
            ...
            def plan_custom():
                device_custom = ophyd.EpicsSignal("EMILEL:test:rdCurE3", name=det_name, labels=("detectors"))
                motor_custom = ophyd.EpicsMotor("IOCsim:m1", name=mot_name, labels=("motors",))
                start = getResult(self.w3)
                stop =   getResult(self.w4)
                number = getResult(self.w5)
                #self.lineEdit.setText("scan {}, please wait...".format(det_name))
